...
def acn_solver(partition, att_idx):
   ...
   s.snapshot = 50000
   s.snapshot_prefix = "temp_models/ACN/rap2_trainval_part%d_%d" % (partition, att_idx)
...
def make_net():
    ...
    for partion in range(1, 6, 1):
        ...
        for idx in range(1,55,1):
            ...
            shuffle = True
            mirror = True
            with open('./prototxts/ACN/%s_%s_%d_%d.prototxt' % (dataset, stage, partion, idx), 'w') as f:
                print(acn(source=source, loss_param=loss_param, batch_size=batch_size, include=include, shuffle=shuffle, mirror=mirror), file=f)
        for idx in range(1,55,1):
            stage = 'test'
            source = './../static/images-list-binary/rap2_%s_%d_att%d.txt' % (stage, partion, idx)
            loss_param = loss_params[idx-1]
