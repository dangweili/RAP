...
if __name__ == '__main__':
    ...
    iter_Cnt = 5
    att_Cnt = 54
    lmdb_path = './../baseline-deepmar-binary/lmdb/'
    for i in range(iter_Cnt):
        test_file = './../static/images-list/rap2_test_%d.txt'%(i+1)
        save_name = './data/sacn_caffenet_%d.mat'%(i+1)
        imgs_name = []
        fid = open(test_file)
        ...
        for j in range(att_Cnt):
            lmdb_name = 'result_ACN_fc8_%d_%d_lmdb'%(i+1, j+1)
            pt_result[:, [j]] = load_results(lmdb_path+lmdb_name, Cnt)
        ##############################################################
