...
% pcaelf, lomo, caffenet_fc6, caffenet_fc7, googlenet_pool5, resnet50_pool5
...
for c_index = 1: length(svm_C)
    for iter = 1: partition_Cnt
        ...
        if strcmp(feature_Norm, 'L2')
            if strcmp(stage, 'trainval')
                store_name = sprintf('%s/model_trainval_%s_%s_%1.2f_part%d.mat', model_path, feature_type, feature_Norm, svm_C(c_index), iter);
            else
                store_name = sprintf('%s/model_%s_%s_%1.2f_part%d.mat', model_path, feature_type, feature_Norm, svm_C(c_index), iter);
            end
        else
            if strcmp(stage, 'trainval')
                store_name = sprintf('%s/model_trainval_%s_%1.2f_part%d.mat', model_path, feature_type, svm_C(c_index), iter);
            else
                store_name = sprintf('%s/model_%s_%1.2f_part%d.mat', model_path, feature_type, svm_C(c_index), iter);
            end
        end
