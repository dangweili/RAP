...
def calculate_accuracy(gt_result, pt_result):
    ...
    label_pos_acc = 1.0*pt_pos/(gt_pos+0.00001)
    label_neg_acc = 1.0*pt_neg/(gt_neg+0.00001)
...
if __name__ == '__main__':
    ...
    net_type = sys.argv[1]
    stage = sys.argv[2]
    test_file = './../static/classification/rap2_ide_att_%s.txt'%(stage)
    fid = open(test_file)
    ...
    # load the predicted results
    db_file = 'lmdb/result_%s_fc8_att_lmdb'%(net_type)
    Cnt = gt_result.shape[0] # number of test images
    pt_result = load_results(db_file, Cnt)
    # savemat('%s.mat'%(stage), {'images_feat': pt_result})
    ...
    fid = open('results/%s/result_%s.pkl'%(net_type, stage), 'wb')
    ...
    fid.close()
    fid = open('results/%s/result_%s.txt'%(net_type, stage), 'w+')
    ...
    fid.close()
    print 'all the results has been saved in results/%s/result_%s.txt!'%(net_type, stage)
