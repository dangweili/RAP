...
fid_train = fopen('classification-control/rap2_ide_att_train.txt', 'w+');
fid_val = fopen('classification-control/rap2_ide_att_val.txt', 'w+');
fid_trainval = fopen('classification-control/rap2_ide_att_trainval.txt', 'w+');
fid_test = fopen('classification-control/rap2_ide_att_test.txt', 'w+'); % consist labeled and unlabeled images
...
for idx_pid = 1:train_ids_cnt
    ...
    tmp = sprintf('%s/%s %d', images_root, images_name{idx_val_img(1)}, person_id);
    for i=1:length(selected_attribute)
...
for idx = 1:length(idx_img_atest)
    tmp = sprintf('%s/%s %d', images_root, images_name{idx_img_atest(idx)}, -1);
    for i=1:length(selected_attribute)
