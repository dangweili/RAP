...
fid_train = fopen('classification/rap2_att_train.txt', 'w+');
fid_val = fopen('classification/rap2_att_val.txt', 'w+');
fid_trainval = fopen('classification/rap2_att_trainval.txt', 'w+');
fid_test = fopen('classification/rap2_att_test.txt', 'w+'); % consist labeled and unlabeled images
...
for idx_pid = 1:train_ids_cnt
    ...
    for idx = 1:length(idx_train_img)
        tmp = sprintf('%s/%s', images_root, images_name{idx_train_img(idx)});
    ...
    tmp = sprintf('%s/%s', images_root, images_name{idx_val_img(1)});
    for i=1:length(selected_attribute)
        tmp = [tmp sprintf(' %d', labeldata(idx_val_img(1), i))];
    end
    tmp = [tmp '\n'];
...
for idx_pid = 1:length(test_ids)
    ...
    for idx = 1:length(idx_img_test)
        tmp = sprintf('%s/%s', images_root, images_name{idx_img_test(idx)});
        for i=1:length(selected_attribute)
...
for idx = 1:length(idx_img_atest)
    tmp = sprintf('%s/%s', images_root, images_name{idx_img_atest(idx)});
    for i=1:length(selected_attribute)
        tmp = [tmp sprintf(' %d', labeldata(idx_img_atest(idx), i))];
    end
    tmp = [tmp '\n'];
    test_set{test_image_cnt} = tmp;
    test_image_cnt = test_image_cnt + 1;
