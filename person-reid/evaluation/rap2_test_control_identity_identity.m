...
feature_attribute_name = sprintf('./features/%s/%s_att_control_identity_instance.mat', test_net, test_alg_type);
test_reid = 1; % whether we should test reid or not
feature_ide_name = sprintf('./features/%s/%s_ide_control_identity_instance.mat', test_net, test_alg_type);

metric_euclidean = 1;
metric_xqda = 1;
metric_kissme = 1;
kissme_pca_dim = -1; % if set -1,  default extract 95% energy
save_mat = sprintf('results/%s/result_algtype%s_testtype%d_control_identity_identity.mat', test_net, test_alg_type, test_type);
